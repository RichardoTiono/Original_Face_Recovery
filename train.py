...
def train_fixer(generator,discriminator, fixer ,rgb_syn_data=None, rgb_real_data=None, read_first=None,
          batch_size = 10, num_epochs = 10, log_path = './results/metadata/', use_cuda=False, wandb=None,
          lr = 0.01, wt_decay=0.005, training_syn=False, last_epoch = None, perceptual_loss=None):
    ...
    # Collect model parameters
    fixer_parameters = fixer.parameters()
    F_optimizer = torch.optim.Adam(fixer_parameters, lr=lr, betas=(0.5, 0.999))
    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        for bix, data in enumerate(syn_train_dl):
            ...
            #Train Fixer
            fake_image = generator(face)
            # a=fake_image[0].view(3,256,256).permute(1,2,0)
            # a=a.detach().cpu().numpy()
            # plt.imshow(a)
            # plt.show()
            out_image = fixer(fake_image)
            #Compute Loss
            f_loss_recon = image_loss(out_image,target)
            ...
            if i % 10 == 0:
                ...
                train_alert ='Iteration: {} Fixer Loss: {}'.format(\
                i, f_loss.item())
                print(train_alert)

                with open(log_path+'val_details.txt', 'a') as f:
                    f.write('\n'+str(i)+','+str(round(f_loss.item())))

                with torch.no_grad():
                    t_cor = predict_fixer(generator,fixer,syn_test_dl,perceptual_loss, train_epoch_num=epoch, use_cuda=use_cuda, 
                                                                                out_folder=out_syn_images_dir + '/test/', iteration = i, suffix='Test')
        ...
        train_alert ='Epoch: {} Fixer Loss: {}'.format(\
        epoch, t_floss / syn_train_len)
        print(train_alert)
        with open(log_path+'details.txt', 'a') as f:
            f.write('\n'+str(epoch)+','+str(round(t_floss / syn_train_len,4)))

        if best_record>(t_floss/syn_train_len): 
            best_record=(t_floss/syn_train_len)
            #save the network
            log_prefix = 'Syn Data'
            #Model saving
            torch.save({'epoch': epoch,
                'fixer_state_dict': fixer.state_dict(),  
                'f_optimizer_dict': F_optimizer.state_dict(),
                }, model_checkpoint_dir + 'fixer'+str(epoch)+'.pkl')
