...
def main():
    ...
    if ON_SERVER:
        ...
        parser.add_argument('--syn_data', type=str, default='./dataset/Syn_data/',
                        help='Synthetic Dataset path')
        parser.add_argument('--celeba_data', type=str, default='./dataset/celeba/',
                        help='CelebA Dataset path')
        parser.add_argument('--sample_data', type=str, default='./dataset/test/',
    ...
    sample_dataset,_ = get_celeba_dataset(dir=sample_data, read_first=read_first, validation_split=0)
    #test_dataset, _ = get_celeba_dataset(read_from_csv=celeba_test_csv, read_first=read_first, validation_split=0)
    
    sample_dl  = DataLoader(sample_dataset, batch_size=1, shuffle=True)
    out_sample_images_dir = sample_data + '/synthesized_sample/'
    #print(out_celeba_images_dir)
    os.system('mkdir -p {}'.format(out_sample_images_dir))
    
    generate_celeba_synthesize(sfs_net_model, sample_dl, train_epoch_num = 0,
